...
function StatPanel({
  ...
  return (
    <div
      className={cn(
        "panel-ornate rounded-game-lg p-4 relative overflow-hidden shadow-emboss",
        bgClass,
        borderClass,
      )}
    >
      <div
        className={cn(
          "absolute top-0 start-0 w-16 h-16 opacity-10 rounded-full blur-2xl",
          color,
        )}
      />
      ...
      <div className="flex items-start justify-between mb-2">
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className="font-heading text-game-2xs uppercase tracking-widest text-game-text-secondary">
              {title}
            </p>
            <p className="font-heading text-game-xs font-bold text-game-text-white">
              {titleHe}
            </p>
          </div>
        </div>
      ...
      <div className="mb-2">
        <p className="text-game-2xs text-game-text-muted font-heading mb-0.5">
          כוח
        </p>
        <p className={cn("text-game-2xl font-display font-bold", colorClass)}>
      ...
      <div className="grid grid-cols-2 gap-1.5 pt-2">
        <div>
          <p className="text-game-2xs text-game-text-muted font-heading">
            {unitLabel}
          </p>
          <p className="text-game-sm text-game-text-white font-semibold tabular-nums">
            {formatNumber(units)}
          </p>
        </div>
        <div>
          ...
          <p className="text-game-sm text-game-text-white font-semibold">
            רמה {level}
          </p>
        </div>
      </div>
...
export default async function BasePage() {
  ...
  const [
    ...
    supabase
      .from("hero")
      .select("mana,mana_per_tick,level")
      .eq("player_id", playerId)
      .single(),
  ]);
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          {
            label: "דירוג עיר",
            value: player.rank_city ? `#${player.rank_city}` : "—",
            icon: "🏆",
            color: "text-game-gold-bright",
          },
          {
            label: "דירוג כללי",
            value: player.rank_global ? `#${player.rank_global}` : "—",
            icon: "🌍",
            color: "text-game-gold-bright",
          },
          {
          ...
          {
            label: "כוח כולל",
            value: formatNumber(player.power_total, true),
            icon: "⚡",
            color: "text-game-gold-bright",
          },
          ...
          <div
            key={label}
            className="card-game p-2.5 text-center shadow-emboss hover:border-game-border-gold/50 transition-colors duration-150"
          >
            <span className="text-xl">{icon}</span>
            <p className="text-game-2xs text-game-text-muted font-heading uppercase tracking-wider mt-1">
              {label}
            </p>
            <p
              className={cn(
                "text-game-lg font-heading font-bold mt-0.5",
                color,
              )}
            >
              {value}
      ...
      {/* ── Resources row (compact, centered) ────────────── */}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 max-w-xl w-full">
          {[
            ...
            },
            {
              ...
              emoji: "🌾",
              color: "text-res-food",
              bg: "border-res-food/20",
            },
          ].map(({ label, value, emoji, color, bg }) => (
            <div
              key={label}
              className={cn(
                "card-game px-2 py-1.5 border shadow-emboss hover:brightness-110 transition-all text-center",
                bg,
              )}
            >
              ...
              <p
                className={cn(
                  "text-game-base font-heading font-bold tabular-nums",
                  color,
                )}
              >
      ...
      <div className="panel-ornate rounded-game-lg p-4 shadow-engrave">
        <div className="panel-header flex items-center gap-1.5 mb-3">
          ...
          <h2 className="font-heading text-game-xs uppercase tracking-wider text-game-gold">
            סיכום צבאי
          </h2>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-x-4 gap-y-3">
          {[
            ...
            <div key={label} className="flex flex-col">
              ...
              <span className="text-game-sm text-game-text-white font-semibold tabular-nums">
                {formatNumber(value)}
              </span>
            </div>
          ))}
