...
describe('BALANCE config smoke — all UI-referenced paths exist', () => {
  ...
  it('combat paths', () => {
    ...
    expect(typeof BALANCE.combat.LOSS_THRESHOLD).toBe('number')
  ...
  it('training paths used by TrainingClient', () => {
    ...
    expect(typeof BALANCE.training.baseCapacity).toBe('number')
    expect(typeof BALANCE.training.capacityPerDevelopmentLevel).toBe('number')
  })
