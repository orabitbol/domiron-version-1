...
const balanceSchema = z.object({
  ...
  combat: z.object({
    ...
    ATTACKER_BASE_LOSS:      z.number(),
    ATTACKER_LOSS_EXPONENT:  z.number(),
    DEFENDER_BASE_LOSS:      z.number(),
    DEFENDER_LOSS_EXPONENT:  z.number(),
    MAX_LOSS_RATE:           z.number(),
    DEFENDER_BLEED_FLOOR:    z.number(),
    ATTACKER_FLOOR:          z.number(),
    BASE_LOOT_RATE:          z.number(),
    LOOT_RATIO_CAP:          z.number(),
    LOOT_RATIO_EXPONENT:     z.number(),
  ...
  training: z.object({
    unitCost: z.object({
      ...
      cavalry:  z.object({ gold: z.number(), capacityCost: z.number(), popCost: z.number() }),
    }),
    enableCavalry:               z.boolean(),
  ...
  tribe: z.object({
    creationManaCost:     z.number().int().min(0),
    taxCollectionHour:    z.number().int().min(0).max(23),
    ...
    spellEffects: z.object({
      ...
      spy_veil:            z.object({ scoutDefenseMultiplier: z.number() }),
      battle_supply:       z.object({ foodReduction:          z.number().min(0).max(1) }),
    }),
